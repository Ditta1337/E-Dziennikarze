...
def solve(groups: list[Group], teachers: list[Teacher],subjects: list[Subject], max_hours_per_day: int,
    ...
    for group in groups:
        for subject in group.subjects:
            # group_id, teacher_id, subject, day, hour
            # print(group.id,subject.teacher.id)
            # print(schedule[group.id, subject.teacher.id, subject.id, :, :])
...
if __name__ == "__main__":
    groups, teachers,subjects, max_hours_per_day = DataParser.parse_input("real_input3.json")
    print(groups)
    print(teachers)
    result = solve(groups, teachers, subjects, max_hours_per_day, WORKING_DAYS)
