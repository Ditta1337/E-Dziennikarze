...
class DataParser:
    ...
    def parse_input(cls, filename: str) -> tuple[list["Group"], list["Teacher"], int]:
        ...
        DataParser._parse_groups(data["groups"])
    ...
    @staticmethod
    def _parse_groups(groups_list) -> None:
    ...
    @classmethod
    def get_subject_by_uuid(cls, subject_uuid: str, hours: int, teacher_uuid: str) -> Subject:
        if not subject_uuid in cls._subjects_by_uuid:
            subject = Subject(subject_uuid, hours, DataParser.get_teacher_by_uuid(teacher_uuid))
    ...
    @classmethod
    def get_group_by_uuid(cls, group_uuid: str, subjects: list[dict]) -> Group:
        if not group_uuid in cls._groups_by_uuid:
            group = Group(group_uuid,
                          [DataParser.get_subject_by_uuid(
