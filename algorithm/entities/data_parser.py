...
class DataParser:
    ...
    def _parse_all_teachers(teachers_list):
        for teacher in teachers_list:
            DataParser._parse_teacher(teacher["teacher_id"], teacher["teacher_name"], teacher["hours"])

    @classmethod
    def _parse_teacher(cls, teacher_uuid: str, name: str, hours: int) -> Teacher:
        if not teacher_uuid in cls._teachers_by_uuid:
            teacher = Teacher(teacher_uuid, name, hours)
